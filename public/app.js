...
app.controller('FlashCardController', function($scope){
	...
	$scope.answerQuestion = function(currentCard){
		// ask about why we need $scope when we are inside the $scope.answerQuestion function
		if (currentCard.correct) $scope.answeredCorrectly = true // did not add an else condition 
		$scope.answered = true
